...
import {IUbiquityBondingV2, IUbiquityBondingShareV2, IUbiquityMetaPool, I3Pool} from "./interfacesV2.sol";
...
abstract contract UbiquityResolver is Helpers, Events {
    ...
    function withdraw(
        ...
        // Get Bond
        IUbiquityBondingShareV2.Bond memory bond = IUbiquityBondingShareV2(ubiquityManager.bondingShareAddress())
        ...
        address bonding = ubiquityManager.bondingContractAddress();
        IUbiquityBondingV2(bonding).removeLiquidity(bond.lpAmount, _bondingShareId);

        // STEP 2 : Withdraw uAD3CRV-f LPs to get back uAD or 3Crv
        // DAI / USDC / USDT / CRV3 / UAD
        if (tok[0] || tok[1] || tok[2] || tok[3] || tok[4]) {
            uint256 amount2 = getTokenBal(TokenInterface(UAD3CRVf));
            IUbiquityMetaPool(UAD3CRVf).remove_liquidity_one_coin(amount2, tok[4] ? 0 : 1, 0);
        }
        ...
        if (tok[0] || tok[1] || tok[2]) {
            ...
            I3Pool(Pool3).remove_liquidity_one_coin(amount1, tok[0] ? 0 : (tok[1] ? 1 : 2), 0);
        }
...
contract ConnectV2UbiquityV2 is UbiquityResolver {
